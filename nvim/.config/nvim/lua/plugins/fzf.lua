return {
  ...
  keys = {
    -- flat scheme matching hers
    { "<leader>f", function() require("fzf-lua").files() end, desc = "Find files" },
    { "<leader>F", function() require("fzf-lua").files({ fd_opts = "--no-ignore --hidden --type f", prompt = "All Files❯ " }) end, desc = "Find all files" },
    { "<leader>b", function() require("fzf-lua").buffers() end, desc = "Find buffers" },
    { "<leader>g", function() require("fzf-lua").live_grep({ prompt = "Grep Project❯ " }) end, desc = "Grep project" },
    { "<leader>G", function() require("fzf-lua").live_grep({ rg_opts = "--hidden --no-ignore --column --line-number --no-heading --color=always --smart-case -g '!.git'", prompt = "Grep All❯ " }) end, desc = "Grep all files" },
    ...
    { "<leader>r", function() require("fzf-lua").resume() end, desc = "Resume last search" },
  },
  config = function()
    ...
    fzf.setup({
      ...
      -- prompt at top, ascending (matches her sorting_strategy = 'ascending')
      winopts = {
        height = 0.85,
        width = 0.85,
        row = 0.35,
        preview = {
          layout = "horizontal",
          vertical = "right:60%",
          delay = 50,           -- snappy preview (hers: timeout 200)
        },
      },
      ...
      keymap = {
        ...
        fzf = {
          ...
          -- her history cycling: C-Up/C-Down through prompt history
          ["ctrl-p"] = "prev-history",
          ["ctrl-n"] = "next-history",
        },
      },
      ...
      files = {
        ...
        path_shorten = 1,       -- her path_display = { truncate = 1 }
      },

      grep = {
        prompt = "Grep❯ ",
        rg_opts = "--hidden --column --line-number --no-heading --color=always --smart-case --sort=path -g '!.git'",
        -- her <C-space> to switch from rg to fuzzy refine
        actions = {
      ...
      buffers = {
        prompt = "Buffers❯ ",
        previewer = false,      -- her buffers picker has previewer = false
      },
      ...
      -- esc closes (her <esc> = actions.close)
      fzf_opts = {
        ["--info"] = "inline-right",
      },
    })
